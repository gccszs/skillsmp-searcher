...
# 列出已安装的技�?python skills/skillsmp-searcher/scripts/install_skill.py list
```

**安装选项�?*
- `query`: 搜索查询�?`.skill` 文件的直接URL/路径
- `--index N`: 安装搜索结果中的第N个技能（默认�?�?- `--page N`: 搜索页码（默认：1�?- `--sort`: 按`stars`（默认）或`recent`排序
